👋 Hi, I'm Yasogan

🎓 Pre-Final Year Computer Science Student
💡 Passionate about Problem Solving, Full-Stack Development & AI

<img align="right" width="300" src="https://i.pinimg.com/originals/47/f0/34/47f0342cec72b800463bf003eac1257e.gif">
🚀 About Me
🌱 Currently improving Data Structures & Algorithms
🤖 Building AI-powered applications
💻 Developing full-stack projects (React + Flask)
🎯 Preparing for product-based companies (Zoho, TCS, Infosys, etc.)
💻 Tech Stack
<p> <img height="40" src="https://img.icons8.com/color/48/python.png"/> <img height="40" src="https://img.icons8.com/color/48/java-coffee-cup-logo.png"/> <img height="40" src="https://img.icons8.com/color/48/html-5.png"/> <img height="40" src="https://img.icons8.com/color/48/css3.png"/> <img height="40" src="https://img.icons8.com/color/48/react-native.png"/> <img height="40" src="https://img.icons8.com/color/48/mysql-logo.png"/> </p>
🧠 Coding Activity
Solving problems daily on LeetCode & coding platforms
Practicing:
Arrays & Strings
Recursion & Backtracking
Linked List, Stack & Queue
Trees & Graphs
🏗️ Projects
🤖 AI Presentation Generator (Flask + React + OpenAI)
🌐 Full-stack Web Applications
⚙️ REST API Development
💪 Strengths
Strong problem-solving skills
Clean and structured coding practices
Quick learner & consistent performer
Ability to build real-world scalable applications
📊 Stats
![LeetCode Stats](https://leetcard.jacoblin.cool/YOUR_USERNAME?theme=dark&font=baloo&ext=contest)

![GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=dark)

![GitHub Streak](https://github.com/Yasogan117&theme=dark)
📬 Contact
📧 your-email@example.com
🔗 GitHub: https://github.com/YOUR_USERNAME

👉 Just replace:

YOUR_USERNAME
Email

If you want, I can make this more attractive with animations, badges, and colors (top GitHub profile style) 🔥
